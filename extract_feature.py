...
def getTestData():
    ...
    offTestData=pd.read_csv('../JData_Action_201604.csv')
    print len(offTestData)
    offTestData=offTestData[(offTestData.time>='2016-04-11')&(offTestData['sku_id'].isin(P))].ix[:,[0,1,2,4]]
    ...
    testFeatures.to_csv("data/testFeatures.csv",index=False)
    testFeatures=testFeatures[testFeatures['sum_type_4']>=1]
    print len(testFeatures)


dataTest=pd.read_csv("data/testFeatures.csv")
dataTest=dataTest[dataTest.sum_type_4>=1].ix[:,[0,1]]
...
def getTrainData():
    ...
    off_train = pd.read_csv('../JData_Action_201604.csv')
    #filter data P
    off_train=off_train[(off_train.time<'2016-04-11')&(off_train['sku_id'].isin(P))]
    print  "JData_Action_201604 record:",len(off_train)
    
    off_train1=pd.read_csv('../JData_Action_201603.csv')
    off_train1=off_train1[(off_train1.time>='2016-03-25')&(off_train1['sku_id'].isin(P))]
    ...
    off_train_pos.to_csv('data/off_train_pos.csv')
    off_train_neg.to_csv('data/off_train_neg.csv')
